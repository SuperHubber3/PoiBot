...
import { addBite } from "../../functions/counters";
...
export default {
    ...
    cooldown: "5s",
    syntaxError: {
        'user': 'Incorrect usage! Use `{PREFIX}`bite {ARGUMENTS}'
    },
    options: [
        { name: "user", description: "User to bite", type: "USER", required: true },
    ],
    ...
    callback: async ({ message, interaction: msgInt, guild, user, args, channel }) => {
        ...
        let mediaString = (new MediaService(CommandType.Bite)).getMedia()
        const bites = await addBite(guild!.id, user.id, target, message)
        let text = `That's ${bites} bites now!`
        if (bites == 1) text = `Their first bite from you!`
        ...
        const embed = new MessageEmbed({ footer: { text } })
            ...
            .setDescription(`*<@${user.id}> bites <@${target}>*`)
